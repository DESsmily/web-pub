...
const { computedFileSize } = require("../utils/fileHandler.js");
...
sshCmd
  .command("add")
  .description(
    "添加ssh连接配置 (web-pub ssh add test 127.0.0.1@22 root 123456)"
  )
  .arguments(
    "<alias> <host@port> <user> <password>",
    "ip@端口（默认22，可不输） 用户名 密码"
  )
...
sshCmd
  .command("del")
  .description("删除ssh连接配置 (web-pub del test)")
...
sshCmd
  .command("list")
  .description("查看ssh连接配置 (web-pub list)")
  .action(async () => {
    checkConfigFile();
    const tabls = [];
    const conf = require("../config.json");
...
program
  .command("use")
  .description("使用ssh配置名称进行连接 部署")
  .arguments(
    "<alias> <remotePath> [localPath]",
    "配置名称 部署路径 （web-pub test /mnt/test [默认当前目录,(./) 可不输]）"
  )
  .action(async (alias, /** @type {string} */ remotePath, localPath) => {
    ...
    if (
      remotePathErr.includes(remotePath) ||
      remotePath.split("/").some((item) => item.includes("\\"))
    ) {
...
program
  .command("run")
  .description("配置ssh进行连接 部署")
  .option(
    "-S, --sshConfig <ip@port@user@pass>",
    "服务器配置 例如 127.0.0.1@22@root@123"
  )
  ...
  .action(async (config) => {
    if (!config.sshConfig || !config.remotePath) {
      console.log(
        pc.red("配置项 --sshConfig或-S 和 --remotePath或-R 不能为空")
      );
      return 0;
    }
    ...
    const remotePathErr = [null, undefined, "", "*", "/", "/*"];
    if (
      remotePathErr.includes(config.remotePath) ||
      config.remotePath.split("/").some((item) => item.includes("\\"))
    ) {
      console.log(pc.red("部署路径不规范"));
      return 0;
    }
...
function runDeploy(
  ...
  fileList.forEach((fileName) => {
    ...
    fileRecords.push({
      ...
      "文件大小（bytes）": file.isFile()
        ? file.size
        : computedFileSize(path.resolve(localPath, fileName)),
    });
  });
